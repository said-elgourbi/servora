...
import com.servora.android.data.jobs.PendingJobAudioNoteEntity
 ...
 * mutations) and the pending evidence records — the photo and audio drafts a technician captured that
 * the backend has not accepted — the stores `docs/architecture/offline-first-architecture.md` §2, §9
 * defines.
 *
...
internal abstract class OfflineDatabase : RoomDatabase() {
    ...
    /** The pending audio records: recordings made on this device the backend has not accepted. */
    abstract fun pendingJobAudioNoteDao(): PendingJobAudioNoteDao
