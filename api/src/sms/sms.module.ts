 ...
 *
 * `BR-019` approves Twilio as the production provider, and `ADR-006` D4 keeps that choice inside
 * this module: the authentication domain depends on `SMS_PROVIDER`, never on a provider. Selection
 * and credentials come from configuration (`SMS_PROVIDER`, `TWILIO_*`), and `loadSmsConfig` fails
 * at startup when a selected provider cannot be configured.
 *
