...
| SMS delivery | `SmsProvider.send({ to, body })` behind the `SMS_PROVIDER` token, bound in `SmsModule` (`api/src/sms/`). `SMS_PROVIDER` selects `noop` (default), `sinch` (credentials required at startup) or `file` (development sink, refused in production); every binding retains and logs nothing |
