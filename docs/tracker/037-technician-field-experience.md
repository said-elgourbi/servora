...
Visit status control, the completion sheet that records `BR-077`'s outcome, the note gate moved onto
`VISIT_ADD_NOTE`, and the transition and the note queued through the existing outbox — (Phase 5a,
2026-09-17) product ownership's amendment to `BR-074`: a technician moves a Visit directly between any
two **working** statuses in either direction, reopening a `COMPLETED` Visit included, with `CANCELED` and
`NO_SHOW` left as office-only, truly terminal destinations — and (Phase 5b, 2026-09-18) the fix for the
defect that device QA surfaced: a session the API would refuse — an office member holding the field
capability, or a technician whose own Visit is not the Job's represented one — was offered the field
action and then told the Visit *"is no longer available"*. The projection now answers that question
(`selectedVisit.fieldActionable`) and Android draws the action from it — and (Phase 5c, 2026-09-18)
**product ownership answered `ADR-019` D7**, recorded as **`BR-093`**: an office member holding the office
capability `JOB_UPDATE` drives the Visit status route without crew membership, completion included, while a
completion still requires `VISIT_RECORD_OUTCOME` of every caller and the projection reports only the
destinations the caller may actually execute. Phase 6 is the product owner's device QA.**
...
| Android: the field action on Job Details (primary action, outcome, note) | **Landed — Phase 5 (2026-09-17)** |
| Android: the four field capabilities in `Permission.kt` and the screen gates | **Landed — `VISIT_VIEW_ASSIGNED` in Phase 4; the three the field action needs in Phase 5** |
| Offline queueing of the field action (idempotency key + conflict policy) | **Landed (Phase 5)** — the transition and the note are queued through the existing outbox (`visit.status.change`, `visit.note.add`) |
...
| 5a | `BR-074`'s free movement — the Visit lifecycle amended by product ownership (`BR-040`) | 3, 5 | business rules, API, contracts, docs, tests |
| 5c | `BR-093` — `ADR-019` D7 answered: office Visit completion without crew membership | 5a, **product decision** | business rules, API, contracts, Android, docs, tests |
...
## Phase 5 — Android: the field action on Job Details (landed 2026-09-17)

**Goal.** The technician's one obvious next action (`BR-012`) on the screen that already exists.

**Landed.** The Visit's own status chip is the control that moves it (`VisitStatusAction`), drawn from
the destinations the API reports in `selectedVisit.allowedStatusTransitions` — the confirmed pattern
tracker 020 established for the Job's status, so no lifecycle rule is re-implemented on the client
(`BR-022`, `BR-041`). Choosing the completing destination opens `VisitCompletionSheet`, which asks for
`BR-078`'s outcome type **and** the summary `BR-077` requires and sends both with the transition in one
request. The three field capabilities (`VISIT_UPDATE_ASSIGNED_STATUS`, `VISIT_ADD_NOTE`,
`VISIT_RECORD_OUTCOME`) gate the control, and the note is now written under `VISIT_ADD_NOTE` or the
office's `JOB_UPDATE` rather than under `JOB_UPDATE` alone. The transition and the note are queued
through the existing outbox (`visit.status.change`, `visit.note.add`) with a `clientOperationId`, the
device `capturedAt` and — for the transition — the `expectedVersion` the technician saw, and what the
queue is still holding is presented beside the Visit (`§7`).

**Offline posture, and why it is send-then-queue.** The Visit transition and the note follow the
pattern the Property lifecycle established for a versioned state-change command
(`data/customers/PropertyRepository.kt`): the action is sent, and only a request the API could not be
**reached** for is queued. A refusal is the backend's answer and is reported as such rather than
queued, because a refusal is what the technician has to act on (`BR-032`). This is what keeps
`BR-070`'s conflict confirmation intact: a `SCHEDULED` destination runs `BR-072`'s availability check,
so a conflict is shown and accepted explicitly and the **same** operation (same idempotency key) is
resent with `confirmConflicts` — which a queued row could not do, because the engine never re-applies
a refused row. A queued action is stored with the version the technician saw, so `ADR-019` D5's
refuse-and-reconcile holds: the API never applies a queued command "as close as possible".
...
| Visit status transition (`PATCH …/status`) | **Adopter — mutation, client and API (Phase 5)** | The route takes a `clientOperationId` and the conflict policy is refuse-and-reconcile (D5 ✓); the client sends it and queues it when the API could not be reached, under the same key, the device `capturedAt` and the `expectedVersion` the screen saw (`visit.status.change`) |
| Visit completion with its outcome | **Adopter — mutation, client and API (Phase 5)** | Same route, same row: the outcome is written with the transition in one transaction, so nothing partial is ever queued |
| Visit note (`POST …/notes`) | **Adopter — mutation, client and API (Phase 5)** | Queued on the note's own idempotency key (`visit.note.add`, `§5`); a note has no version, so idempotency alone covers a repeat |
| Visit field action pending overlay | **Adopter — presentation (`§7`)** | The queued transition and notes are read back from the outbox and shown beside the represented Visit, with the queue's own state and, for a refused row, its reason and an explicit discard (`BR-014`, `BR-032`) |
...
| 5 | 2026-09-17 | **The field action on the Android Job Details screen.** The three field capabilities `VISIT_UPDATE_ASSIGNED_STATUS`, `VISIT_ADD_NOTE` and `VISIT_RECORD_OUTCOME` joined `Permission.kt` and `customerPermissionsUiState`, and the screen's gates are drawn on them rather than on the office's `canUpdateJob`. `JobDetailsVisitDto` / `JobDetailsVisit` gained `allowedStatusTransitions`, so the client holds the API's answer instead of a second copy of the lifecycle (`BR-022`, `BR-041`). **`VisitStatusAction`** makes the Visit's own status chip the control that moves it — the pattern tracker 020 confirmed for the Job's status — and its menu lists exactly the destinations the backend reported; the completing destination opens **`VisitCompletionSheet`**, which asks for `BR-078`'s outcome type and the summary `BR-077` requires and sends both with the transition in one request. `BR-075`'s correction is offered as the destination it is. **`VisitFieldActions`** and **`VisitFieldActionHandler`** queue `visit.status.change` and `visit.note.add` through the existing `OutboxStore` / `OutboxReplayEngine` with a `clientOperationId`, the device `capturedAt` and the transition's `expectedVersion`, and `VisitFieldPendingNotice` shows what the queue is still holding beside the Visit, with the queue's own state and an explicit discard for a refused row. The action is **sent first and queued only when the API could not be reached** — the Property lifecycle's shape for a versioned state-change command — which is what keeps `BR-070`'s conflict confirmation working: a conflict is shown and accepted explicitly and the same operation is resent with `confirmConflicts`, which a refused outbox row could never do. The note is now written under `VISIT_ADD_NOTE` **or** `JOB_UPDATE` (`docs/api/job-actions.md` §2), so a technician can write one without the office capability. `JobDetailsRepository.appliedOperations` re-reads the Job when the engine reports an applied operation, so a queued action stops being presented as waiting the moment the backend answers. EN/FR copy for every new string. | `android/app/src/main/java/` (`domain/auth/Permission.kt`, `domain/model/VisitOutcome.kt` (new), `data/jobs/VisitFieldOffline.kt` (new), `data/jobs/VisitFieldActions.kt` (new), `data/jobs/VisitFieldActionHandler.kt` (new), `domain/model/JobDetails.kt`, `data/jobs/JobDetailsDtos.kt`, `JobActionDtos.kt`, `JobDetailsApi.kt`, `JobDetailsResult.kt`, `JobDetailsRepository.kt`, `di/JobsOfflineModule.kt`, `ui/customers/CustomerPermissionsUiState.kt`, `ui/jobs/JobDetailsUiState.kt`, `JobDetailsViewModel.kt`, `JobDetailsActions.kt`, `JobDetailsScreen.kt`, `ui/navigation/ServoraNavHost.kt`), `res/values*/strings.xml`, tests (`data/jobs/JobDetailsRepositoryTest.kt`, `ui/jobs/JobDetailsViewModelTest.kt`, `ui/jobs/JobDetailsScreenTest.kt`, `PhotoUploadApi.kt` and the two other fakes) | **Tier B, because the phase completes.** **Compile: PASS** (`compileDebugKotlin`). **Unit: PASS** (`testDebugUnitTest` — 601 tests, 0 failures, including the new cases: the destinations mapping, the transition's destination/provenance/version, the outcome in the same request, queue-on-undelivered with the key kept, a refusal reported rather than queued, the ViewModel's destination/outcome/queue/conflict-resend/applied-re-read/discard, and the screen's capability gates, its menus and the completion sheet's two required fields). **Device-test sources: PASS** (`assembleDebugAndroidTest`; **no `adb` and no device command was run**, `qa.md` §7.3 — the new screen tests are the product owner's to run). **Lint: PASS** (`lintDebug`). Two defects the tests caught and that were fixed rather than asserted around: the Visit's allowed destinations were parsed but never mapped into the domain model, and the outcome summary was sent untrimmed. |
| 5a | 2026-09-17 | **`BR-074`'s free movement — a product-owner amendment to the Visit lifecycle that Phases 3 and 5 built.** Product ownership confirmed that the next-status chain did not account for the reality of field work: a technician must be able to see **every** Visit status and move a Visit directly between any two working statuses, in either direction, in one operation — catching up after a missed tap, stepping backward, and **reopening** a `COMPLETED` Visit completed by mistake or one whose work must continue. `VISIT_STATUS_TRANSITIONS` now reads the way `JOB_STATUS_TRANSITIONS` already does, so the technician's menu widened with **no Android production change**: `VisitStatusAction` renders exactly the destinations the API reports (`BR-041`). Reopening preserves the completion and its outcome in append-only history while **clearing the Visit's current outcome**, so the next completion must record a **new** one (`BR-077`, `BR-079`) — the four columns are cleared together, as their pair checks require. `ADR-019` D4 gained `BR-061`'s invariant **read in the other direction**: a Visit moved into a working status while its Job is `PENDING_REVIEW` takes the Job back to `IN_PROGRESS`, so an actively worked Visit can never coexist with a Job awaiting completion review — which is what makes reopening safe rather than a hole in `BR-061`. `CANCELED` and `NO_SHOW` were deliberately **left unchanged**: office-only dispatch actions with a structured reason (`BR-066`, `BR-076`), truly terminal, destinations of no field route. Ordinary status changes and reversals require **no** reason dialog. **No migration:** `visits_status_check`, `visit_status_history`'s checks and `visits_completed_outcome_check` already permit every transition and the cleared outcome — the transitions were never a schema constraint. `is_correction` deliberately stays as narrow as the `EN_ROUTE → SCHEDULED` case `BR-075` named, because `visit_status_history_correction_check` declares exactly that pair and every status event already carries the previous status, the new status, the actor and the timestamp `BR-067` requires; generalizing it is a recorded deferral, not an omission. **Changed:** `.clinerules/Business Rules.md` (`BR-074`, `BR-075`, `BR-061`, `BR-066`, `BR-078`, `BR-079`, each with its `BR-040` record) | `api/src/jobs/job.types.ts`, `visit-job-consequence.ts`, `jobs.service.ts`, `job-details.dto.ts`, `src/jobs/job.types.spec.ts`, `src/jobs/visit-job-consequence.spec.ts`, `src/jobs/job-details.dto.spec.ts`, `api/test/visit-field-lifecycle.e2e-spec.ts`, `docs/api/job-actions.md` §7.2/§7.4, `docs/domain/job-visit-domain-model.md` §8.7/§9.1/§9.5/§11.1/§11.2, `docs/decisions/019-technician-field-experience.md` (D3/D4 update), this tracker | **Tier A, with an escalation to Tier B for the API** (`qa.md` §3.1: the change crosses a contract and a domain invariant). **Typecheck: PASS** (`npm run typecheck` — two errors in `test/technician-home.e2e-spec.ts` are **pre-existing on this branch** and were confirmed by stashing the change: `'visit.scheduledEnd' is possibly 'null'`; they are unrelated and were not introduced here). **API unit: PASS** (`npx vitest run src/jobs/job.types.spec.ts src/jobs/visit-job-consequence.spec.ts src/jobs/job-details.dto.spec.ts src/jobs/job-action.dto.spec.ts` — 44 tests). **API e2e: PASS** (`npm run test:e2e -- test/visit-field-lifecycle.e2e-spec.ts test/job-actions.e2e-spec.ts test/job-details.e2e-spec.ts test/job-activity.e2e-spec.ts test/technician-home.e2e-spec.ts` — 90 tests against the running foundation stack; new coverage: the refusal set is now only standing still and the dispatch pair, a skipped step (`SCHEDULED → IN_PROGRESS`) and a step backward (`ON_SITE → EN_ROUTE`) each recorded once with no Job consequence, and a full reopen cycle asserting the cleared outcome columns, the preserved `visit_outcome_history` row, `PENDING_REVIEW → IN_PROGRESS`, and that the next completion must supply a **new** outcome). **Android: no production change required** — the menu is the API's list; the existing Android unit suite was re-run to confirm no regression |
| 5b | 2026-09-18 | **The field action is offered only to a caller the API will accept — a defect found in the product owner's own device QA of Phase 5.** Symptom: completing a Visit shown as **Overdue** answered *"This job or visit is no longer available."* Reproduction against the running stack: the seeded **Manager** role holds the **whole** permission catalogue, so an office session holds `VISIT_UPDATE_ASSIGNED_STATUS` — which admits it to `PATCH /jobs/:id/visits/:visitId/status` and made the screen offer the action — while `changeVisitStatus` applies the crew scope to **every** caller (`ADR-019` D3, the Phase 3 note below), so the Visit was refused `404 VISIT_NOT_FOUND`, which Android maps to `job_action_error_not_found`. Nobody was wrong on their own: the route's guard and its scope answer two different questions, and the projection reported only `allowedStatusTransitions` — the *structural* destinations — with no answer to "may **this caller** drive this Visit", which no client can derive (a client knows its user, not its membership). The same shape traps a technician admitted through **another** Visit of the same Job: `BR-081` selects one Visit for the Job, so the screen can present a colleague's. **Fix, without deciding anything:** `JobDetailsVisitDto.fieldActionable` (`ADR-019` D3's own scope, answered from the crew the read already holds and the membership the request was authorized for — `JobDetailsReadOptions.callerMembershipId`), and Android draws the Visit action from it: `JobDetailsUiState.canChangeVisitStatus` requires it, and the note kind follows the API's own split (`JOB_UPDATE` keeps the office reach, `VISIT_ADD_NOTE` is scoped to the crew). **`ADR-019` D7 stays open and the API still refuses an office caller**: the projection reports what the route enforces rather than inventing an answer (`BR-042`), which is what makes the refusal honest instead of a lie about a Visit that exists. The old cached-response and offline path is safe by construction: `JobDetailsVisitDto.fieldActionable` defaults to `false`, so a payload written before this field never offers an action the API would refuse (`BR-013`, `BR-014`). **No migration, no route change.** | `api/src/jobs/job-details.dto.ts`, `jobs.controller.ts`, `job-details.dto.spec.ts`, `api/test/visit-field-lifecycle.e2e-spec.ts`, `android/…/data/jobs/JobDetailsDtos.kt`, `JobDetailsRepository.kt`, `…/domain/model/JobDetails.kt`, `…/ui/jobs/JobDetailsUiState.kt`, `JobDetailsScreen.kt`, `JobDetailsRepositoryTest.kt`, `JobDetailsViewModelTest.kt`, `JobDetailsScreenTest.kt`, `docs/api/job-details.md` §3.1/§3.2, `docs/api/job-actions.md` §7, this tracker | **Tier A escalated to Tier B for the API** (`qa.md` §3.1: the change crosses a shared contract). **API typecheck: PASS for this change** — `npm run typecheck` reports only the two `test/technician-home.e2e-spec.ts` errors Phase 5a recorded as pre-existing; the two `visit-field-lifecycle.e2e-spec.ts` errors a hoisted `const crew` had introduced were fixed with it (`roleCode: 'LEAD' as const`). **API unit: PASS** (`npx vitest run src/jobs/job-details.dto.spec.ts` — 9 tests, 2 new). **API e2e: PASS** (`npm run test:e2e -- test/visit-field-lifecycle.e2e-spec.ts` — 27 tests, 2 new: a technician admitted through another Visit and an office caller holding the field capability each read `fieldActionable: false` with the destinations still reported, and the action is still refused `404`; the read-capability case asserts `true`). **Android: NOT RUN** — no Gradle/Android command is run on this machine at the product owner's request, so the two JVM specs and the device-test source compile are the product owner's to run (`./gradlew testDebugUnitTest --tests 'com.servora.android.ui.jobs.*'`, `./gradlew assembleDebugAndroidTest`). No daemon was started, and the temporary reproduction spec written for the diagnosis was deleted | 
| 5c | 2026-09-18 | **`ADR-019` D7 answered by product ownership — office Visit completion, recorded as the new rule `BR-093` (`BR-040`).** The office half of the Visit lifecycle was the one thing this slice left open: D7 recorded *whether* an office member may perform a field action and decided nothing, so the status route was field-only and `fieldActionable` reported `false` to every office caller — Phase 5b's honest refusal. Product ownership decided it: **an authorized office member may complete a Visit from Job Details without being on its crew**, because a technician who telephones dispatch, one who loses connectivity or their device, a Visit left open by accident, and post-hoc clean-up after the work was confirmed finished are all real cases and none of them should force a member onto a crew. `PATCH /jobs/:id/visits/:visitId/status` now declares `@RequireAnyPermission(JOB_UPDATE, VISIT_UPDATE_ASSIGNED_STATUS)` and the **scope follows the capability that admitted the caller**: the office capability (`JOB_UPDATE`, which every other office Job/Visit action already requires) addresses the organization's Visits **without** crew membership, while a field capability keeps the caller bounded by their own **current** crew (`ADR-019` D2, D3) — so a technician's authorization is not weakened and a caller without the office capability gains nothing from the decision. The completion keeps its own question: `VISIT_RECORD_OUTCOME` is required of **every** caller, the office included, so the office capability admits the caller and the outcome capability still authorizes the outcome. `selectedVisit.fieldActionable` now means "this caller is authorized to drive this Visit" (crew **or** office capability), and `selectedVisit.allowedStatusTransitions` is narrowed to the destinations the caller may actually execute — **none** for a caller who may drive neither (which supersedes Phase 5b's disclosure of the structural list to a caller the route would refuse) and no `COMPLETED` without the outcome capability. The actor was already the authenticated member on `visit_status_history`/`visit_outcome_history`, so an office completion names the office member and never the crew's Lead. **No migration, no new capability** — the capabilities and the append-only history already existed (`BR-042`) — and Android needed one gate: `JobDetailsScreen` required `canUpdateAssignedVisit`, and now accepts the office capability too, or a session holding only it would be hidden by the client although the API accepts it (`BR-007`, `BR-011`). | `api/src/jobs/jobs.controller.ts`, `api/src/jobs/jobs.service.ts`, `api/src/jobs/job-details.dto.ts`, `api/src/jobs/job-details.dto.spec.ts`, `api/test/visit-field-lifecycle.e2e-spec.ts`, `android/…/ui/jobs/JobDetailsScreen.kt`, `…/androidTest/…/ui/jobs/JobDetailsScreenTest.kt`, `…/test/…/ui/jobs/JobDetailsViewModelTest.kt`, `.clinerules/Business Rules.md` (`BR-093`, `BR-066`, `BR-075`), `docs/decisions/019-technician-field-experience.md`, `docs/api/job-actions.md` §2/§7, `docs/api/job-details.md` §3.2, `docs/domain/job-visit-domain-model.md` §8.3/§9.1, this tracker, `README.md` | **Tier B for the API** (`qa.md` §3.1: the change is to authorization and crosses a shared contract). **API typecheck: PASS for this change** — `npm run typecheck` reports only the two `test/technician-home.e2e-spec.ts` errors Phase 5a recorded as pre-existing. **API unit: PASS** (`npx vitest run src/jobs/job-details.dto.spec.ts` — 9 tests, including the new office-authorization and destination-narrowing assertions). **API e2e: PASS** (`npm run test:e2e -- test/visit-field-lifecycle.e2e-spec.ts` — 29 tests, 2 new and 2 changed: an office member completes a Visit they are on no crew of and **both** history rows name them, an office caller without `VISIT_RECORD_OUTCOME` is offered every other destination and refused `403` on the completion, a session holding the field capabilities but not the office one stays crew-scoped `404`, and a caller who may drive neither is offered no destination). **Android: NOT RUN** — no Gradle/Android command is run on this machine at the product owner's request, so the new Compose test, the changed JVM test and the device-test source compile are the product owner's to run (`./gradlew testDebugUnitTest --tests 'com.servora.android.ui.jobs.*'`, `./gradlew assembleDebugAndroidTest`). No daemon was started. |
...
1. **The note route's authorization was decided by product ownership on 2026-09-17**, not by the
   ...
   question and is neither implemented nor blocked. **(Superseded on 2026-09-18: Phase 5c answers D7,
   recorded as `BR-093` — the office capability drives the status route without crew membership — and the
   office half is implemented.)**
